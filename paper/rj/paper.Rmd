---
title: "nowcast: Economic Nowcasting with Bridge Equations and Real-Time Evaluation in R"
author:
...
abstract: >
  The \CRANpkg{nowcast} package provides bridge-equation nowcasting in
  R. It estimates current-quarter values of a low-frequency
  macroeconomic variable (typically GDP) from higher-frequency
  indicators (retail sales, industrial production, payrolls, sentiment)
  that are released weeks or months ahead of the official target.
  Mixed-frequency alignment handles the ragged edge of real-time data.
  Pseudo-real-time backtesting over expanding or rolling windows
  produces an out-of-sample performance record, and the
  Diebold-Mariano test with the Harvey-Leybourne-Newbold finite-sample
  correction compares competing specifications on the same target. The
  package is pure R with \CRANpkg{cli} as the only non-base import, is
  available on CRAN, and plugs into any source of quarterly or monthly
  data. Source at
  \url{https://github.com/charlescoverdale/nowcast}.
preamble: |
...
bridge equations within the tidyverse ecosystem. \CRANpkg{midasr}
covers the related MIDAS family of mixed-frequency models.
\CRANpkg{dfms} fits dynamic factor models in C++ at scale. None of
these covers the full evaluation pipeline that a nowcasting analyst
needs: pseudo-real-time backtesting with expanding or rolling
windows, and pairwise Diebold-Mariano testing with the
Harvey-Leybourne-Newbold finite-sample correction for small samples.
\CRANpkg{nowcast} fills that gap.

## Background

**The mixed-frequency problem.** A quarterly target (GDP) must be
aligned with monthly indicators. The standard choice, following
@mariano2003new, is to aggregate monthly indicators to a quarterly
...
**Bridge equations.** A bridge equation regresses the low-frequency
target on the quarterly averages of monthly indicators
[@baffigi2004bridge]. Optionally an autoregressive term in the
...
press-releases report. @schumacher2016comparison compares bridge to
MIDAS on a common dataset and finds comparable out-of-sample
accuracy with bridge's simpler specification; @clements2008macroeconomic
reach a similar conclusion for US GDP. @foroni2013survey provide a
survey of mixed-frequency econometrics covering both approaches.

**The ragged edge.** Different indicators have different publication
...
model must handle this. @wallis1986forecasting formalised the
...
**Real-time evaluation.** In-sample fit is a weak guide to nowcast
...
pseudo-real-time evaluation [@stark2002database]: for each target
...
the estimation sample length. @giannone2008nowcasting standardised
this approach. @aruoba2009real construct a related continuous-time
index of US business conditions that has become the canonical
dynamic-factor comparator for any single-equation nowcast.
@bok2018macroeconomic survey the modern nowcasting literature,
including the New York Fed Nowcast (the institutional benchmark for
US GDP nowcasting).

**Comparing nowcasts.** The Diebold-Mariano test
[@diebold1995comparing] compares loss differentials between two
forecast series against the null of equal predictive accuracy.
@harvey1997testing showed that the original test is over-sized in
...
@giannone2008nowcasting. The \code{start} argument sets the first
...
@harvey1997testing showed that the asymptotic test is over-sized in
...
## Case study: the 2020 pandemic quarter-by-quarter

The expanding-window backtest provides a natural narrative window
for the 2020 pandemic shock, the largest peacetime peacetime
real-GDP contraction in US history. Evaluated quarter by quarter,
the bridge nowcast returned:
...
1. \CRANpkg{nowcast} implements bridge equations only. Mixed-data
   sampling (MIDAS) regressions of the @ghysels2004midas family, and
   dynamic factor models in the @giannone2008nowcasting tradition,
   are out of scope; users needing those should look to
   \CRANpkg{midasr} and \CRANpkg{dfms} respectively.
2. The package does not fetch data. Users supply a data frame of
...
3. Real-time data revisions are not tracked. The package operates
...
4. Flash-estimate indicators with intra-month release dates require
   manual alignment. The monthly aggregation in \code{nc\_align()}
   assumes one observation per month.
5. The AR specification in \code{nc\_bridge()} is restricted to
...
**Bridge equation with AR($p$) term.** Let $y_t$ denote the
...
**Mincer-Zarnowitz regression.** For realised $y_t$ and forecast
...
**Diebold-Mariano test with HLN correction.** Let
...
variance estimate. @harvey1997testing show this is over-sized in
...
handling. Contributions are welcome at
\url{https://github.com/charlescoverdale/nowcast}.
